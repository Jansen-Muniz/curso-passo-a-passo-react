...
const Header = () => (
  <header>
    <img
      className="logo"
      src="img/logo-burgao-do-ze.jpg"
      alt="burgão do Zé"
    />
  </header>
)

const Burger = ({ img, alt, name, ingredients, soldOut, price }) => (
  ...
  <main className="menu">
    {burgers.length > 0 && (
      <>
        <h2>Cardápio</h2>
        <p>
          Depois de uma semana desafiadora, nada melhor do que saborear
          um burger onde cada mordida é um abraço saboroso, não é mesmo?🍔✨
        </p>
...
const OrderMsg = ({ closeHour }) => {
  ...
  return (
    <div className="order">
      <p>
        🕛 Tamo aberto até as {closeHour}h. Vem visitar a gente ou faça seu pedido online.
      </p>
      <button className="btn" onClick={handleClick}>Fazer pedido</button>
    </div>
  )
}
    ...
    <footer className="footer">
      {
        isOpen
          ...
          : <p>🕛 Vamo abrir amanhã as {openHour}h</p>
      }
    </footer>
  )
}
